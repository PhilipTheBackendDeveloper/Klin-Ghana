import { handleCreateTeamMember, ADMIN_CORS_HEADERS } from '../../src/server/adminHandler';

export default async function handler(req: any, res?: any) {
  const isWebStandard = !res || typeof res.writeHead !== 'function';

  if (req.method === 'OPTIONS') {
    if (isWebStandard) return new Response(null, { status: 200, headers: ADMIN_CORS_HEADERS });
  ...
  if (req.method !== 'POST') {
    ...
    if (isWebStandard) return new Response(JSON.stringify(body), { status: 405, headers: ADMIN_CORS_HEADERS });
    res.writeHead(405, ADMIN_CORS_HEADERS);
  ...
  const result = await handleCreateTeamMember(headers, rawBody);
